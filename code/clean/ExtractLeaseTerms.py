...
def parallelize(df, function=apply_extract_term):
	'''
	tool to apply a function in parallel to a DataFrame
	
	df : DataFrame
		data to which function should be applied
	function: func 
		function to apply
	'''
	# Split the DataFrame into smaller chunks
	n_chunks = cpu_count() # or specify any other number
...
if __name__ == "__main__":
	...
	###########################
	print("Extracting Open Lease Titles:")
	file = os.path.join(raw_folder, 'hmlr','LEASES_FULL_2023_07.csv')
	df = pd.read_csv(file)
	...
	# # Change only files
	# ###########################
	# print("Extracting Change Only Files:")
	# dfs = []
	# folder = os.path.join(raw_folder, "hmlr", "cou")
	# files = [file for file in os.listdir(folder) if file.endswith('csv')]
	# for file in files:
	#     print(file)
	#     file = os.path.join(folder, file)
	#     this_df = pd.read_csv(file, encoding='ISO-8859-1')
	#     dfs.append(this_df)
	# df = pd.concat(dfs, ignore_index=True)
	# df = df.rename(columns={'Date of Lease' : 'date_registered', 'TERM':'Term'})

	# print(df)

	# df[['number_years','date_from','date_to']] = df.progress_apply(lambda row: extract_term(row['Term'], date_registered=row['date_registered']), axis=1, result_type="expand")
	# output_file = os.path.join(working_folder, 'cou_extracted_terms.csv')
	# df.to_csv(output_file)
